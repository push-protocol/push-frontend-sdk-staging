...
export default class CalendarEvents extends React.Component<any> {
  ...
  handleUrlPress(matchingString: string, matchIndex:number/*: number*/) {
    let pattern = /\[([^:]+):([^\]]+)\]/i;
  ...
  renderStyles(matchingString: string, matches: string[]) {
  ...
  renderThreeStyles(matchingString: string, matches: string[]) {
    // matches => ["[@michel:5455345]", "@michel", "5455345"]
    let pattern = /\[([^:]+):([^\]]+)\]/i;
    let match = matchingString.match(pattern) || [];
